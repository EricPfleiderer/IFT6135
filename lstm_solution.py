...
class LSTM(nn.Module):
    ...
    def loss(self, log_probas, targets, mask):
        ...
        # running_loss = 0
        # for i, example_i in enumerate(log_probas):  # For every example in the batch
        #     for j, sequence_j in enumerate(example_i):  # For every sequence in the example
        #         # Sequence_j has dimension (N, ) and is a prob distribution over every word in the vocab
        #         # The index we are interested in is the one that corresponds to the target
        #         target_index = targets[i][j]
        #
        #         # The loss is the negative log of the prediction at said index
        #         running_loss += - torch.log(sequence_j[target_index])
        #
        # # Average over number of examples, sequence length and vocabulary size
        # running_loss /= np.prod(log_probas.shape)
        #
        # return running_loss
